    ...
    while True:
        ...
        # try:
            ...
            for query_data in queries:
                ...
                clicks_response = send_clicks_with_rocket_boost(token)
                if cek_rocket == 'y':
                    print(Fore.YELLOW + Style.BRIGHT + f"\r[ Rocket ] : Checking...", end="", flush=True)
                    # time.sleep(3)
                    response = activate_rocket(token)
                    print(response)
                    if response['status_code'] == 400:
                        ...
                        print(Fore.GREEN + Style.BRIGHT + f"\r[ Rocket ] : {clicks_response}        ", flush=True)
                        print(Fore.GREEN + Style.BRIGHT + f"\r[ Rocket ] : You got {total_coin}         ", flush=True)
                        ...
                        print(Fore.YELLOW + Style.BRIGHT + f"\r[ Rocket ] : Tapping...",  flush=True)
                        for _ in range(100):
                            clicks_response = send_clicks_with_rocket_boost(token)
                            response = activate_rocket(token)
